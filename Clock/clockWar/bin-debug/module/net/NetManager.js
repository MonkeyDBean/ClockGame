...
var NetManager = (function () {
    ...
    p.GetFriendUid = function () {
        ...
        var urlreq = new egret.URLRequest(NetAPI.api["addfriend"]);
     ...
     * 从玩家信息Json文件提取数据
     */
    p.PickPlayerDataFromJson = function () {
    ...
    p.onPostPlayerScoreComplete = function (e) {
        var playerRank = JSON.parse(e.target.data.toString());
        var allPlayerCount = parseInt(playerRank["total"]);
        var nowPlayerCount = parseInt(playerRank["rank"]);
    ...
    p.onPostPlayerScoreError = function (e) {
        this.CalculateRank();
        this.SubmitPlayerDetail();
    };
    /**
     * 本地计算排名
     */
    p.CalculateRank = function () {
        var norm = jStat.ztest(this.submitScore, Info.averageScore, Info.standardDeviation, 1);
     ...
     * 获取是否 关注公众号
     */
    p.GetIfFollowedServer = function () {
    ...
    p.onFollowedServerComplete = function (e) {
        console.log(e.target.data.toString());
        var receiveStr = e.target.data.toString();
        var followedJson;
        if (receiveStr.indexOf("head") != -1 && receiveStr.indexOf("body") != -1) {
            var aux = receiveStr.split("<body>")[1];
    ...
    p.onSysTimeLoadComplete = function (e) {
        console.log(e.target.data.toString());
...
egret.registerClass(NetManager,'NetManager');
